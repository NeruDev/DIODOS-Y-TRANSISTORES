...
exam_content = r"""# Formulario de Examen: Tema 3 (Transistor Unipolar FET y MOSFET)
...
| **Saturación**| $V_{DS} \ge V_{GS} - V_{GS(Th)}$ | Fuente de Corriente Constante ($I_D = k(V_{GS}-V_{Th})^2$) |
...
4. **Calcula** $I_D$ usando Shockley o la ecuación $k$ del E-MOSFET. (Resuelve la cuadrática y escoge la raíz físicamente posible: para JFET canal N, $V_P < V_{GS} < 0$).
...
print("Formulario-tema_3-examen.md overwritten successfully.")
